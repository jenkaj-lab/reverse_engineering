...
2. [[RmGetList]] gets the exact number of applications using the file, allocates the required memory, then performs a second call to [[RmGetList]] to retrieve information on each application
3. [[RmShutdown]] requests the shutdown of any processes and services using the target file
